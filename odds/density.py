...
class MDEFKDE(BaseDetector):
    ...
    def __init__(self, k_sigma: int, R: Union[float, str], alpha: int, sample_size: int, win_size: int):
...
class ExactMDEF(BaseDetector):
    def __init__(self, k_sigma: int, R: float, alpha: int, win_size: int, M: int = 5):
        assert alpha >= 1, "alpha should be set greater than 1."
        self.p = None
        self.k = k_sigma
        self.R = R
    ...
    def score_samples(self, x):
        ...
        for i, xx in enumerate(x):
            mtp = MTreePoint(xx)
            neighbours_at_R = self.mt.root.range_query(mtp, self.R)
            if len(neighbours_at_R) != 0:
                neighboring_count = self.mt.root.score_point(mtp, self.r)
                neighboring_counts_for_neighbours = [self.mt.root.score_point(p, self.r) for p in neighbours_at_R]
                mean_ncfn = np.mean(neighboring_counts_for_neighbours)
                std_ncfn = np.std(neighboring_counts_for_neighbours)
                # If mean_ncfn == 0, score should be set equal to 1:
                if mean_ncfn != 0:
                    mdef = 1 - (neighboring_count / mean_ncfn)
                    std_mdef = std_ncfn / mean_ncfn
                    res[i] = mdef - self.k * std_mdef
        return res
    ...
    def copy(self):
        raise NotImplementedError("The copy method for Exact MDEF has not been implemented yet.")
